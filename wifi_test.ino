...
#define WLAN_SSID       "admiral_quackbar"        // cannot be longer than 32 characters!
#define WLAN_PASS       "brolaf777"
...
void setup(void)
{
  ...
  */
  ...
  //try to connect to the Raspberry Pi server so we can upload data
  uint32_t server_ip = cc3000.IP2U32(192, 168, 0, 12);
  int server_port = 69;
  int buffer_size = 16; //must be >= buffer array allocation below
  
  char buffer[16];
  
  Serial.println(F("Connecting to Raspberry Pi data server..."));
  cc3000.printIPdotsRev(server_ip);
  Serial.println("...");

  Adafruit_CC3000_Client client = cc3000.connectTCP(server_ip, server_port);
  if(client.connected()) {
    Serial.println("connected!!! :)");
    
    //ok, start collecting and sending data
    while(client.connected()) {
      uint32_t sensor_data[3];
      sensor_data[0] = random();
      
      Serial.print("moisture reading: ");
      Serial.println(sensor_data[0]);
      
      //fill the buffer w/ our data
      //sensor_data[0] = moisture_value;
      
      //write to the tcp client (sending it to the server we're connected to)
      client.write(sensor_data, buffer_size);
      delay(1000);
    }
  }
